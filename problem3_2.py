...
@njit
def probability_model1_data_and_theta(x1, x2, x3, y1, y2, y3, sigma,tau,theta):
    #den = (2*np.pi)**(3/2) * (sigma**2) * tau
    exponent = -0.5*((y1-theta*x1)**2/sigma**2 + (y2-theta*x2)**2/sigma**2 + (y3-theta*x3)**2/tau**2 )
    return exponent 

@njit
def probability_model2_data_and_theta_psi(x1, x2, x3, y1, y2, y3, sigma,tau,theta,psi):
    #den = (2*np.pi)**(3/2) * (sigma**2) * tau
    exponent = -0.5*((y1-theta*x1 - psi*x1**2)**2/sigma**2 + (y2-theta*x2 - psi*x2**2)**2/sigma**2 + (y3-theta*x3 - psi*x3**2)**2/tau**2 )
    return exponent
...
def theta_sampling_model1(x1, x2, x3, y1, y2, y3, sigma, tau,num_samples=1000,burn_in=50):
...
def gibbs_sampling_model2(x1, x2, x3, y1, y2, y3, sigma, tau,ys,num_samples=1000,burn_in=50):
...
def compute_proxi_volume(x1, x2, x3, y1, y2,sigma,tauS, yS,Delta):
    ...
    for i in range(len(x3)):
        ...
        p_M1 = probability_model1_data_and_theta(x1, x2, x3[i], y1, y2, y3[i], sigma,taus[i],thetas_model1)
        p_M2 = probability_model2_data_and_theta_psi(x1, x2, x3[i], y1, y2, y3[i], sigma,taus[i],psis_model2,thetas_model2)
        logBs = p_M2 - p_M1
...
Delta = 0.5
x2 = np.linspace(0.1, 3.1, 50)


yS_values = np.linspace(linear_(0, theta), linear_(3, theta), 10)
volumes = []
